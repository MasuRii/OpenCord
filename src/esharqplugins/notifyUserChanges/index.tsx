...
function triggerVoiceNotification(userId: string, userChannelId: string | null) {
    ...
    const title = shouldBeNative() ? t(`غيّر ${name} حالته الصوتية`, `User ${name} changed voice status`) : t("تغيّر حالة صوتية لمستخدم", "User voice status change");
    if (userChannelId) {
        if (userChannelId !== myChanId) {
            showNotification({
                title,
                body: t("انضمّ إلى قناة صوتية", "joined a new voice channel"),
                noPersist: !settings.store.persistNotifications,
                richBody: getRichBody(user, t(`انضمّ ${name} إلى قناة صوتية`, `${name} joined a new voice channel`)),
            });
        }
    } else {
        showNotification({
            title,
            body: t("غادر قناته الصوتية", "left their voice channel"),
            noPersist: !settings.store.persistNotifications,
            richBody: getRichBody(user, t(`غادر ${name} قناته الصوتية`, `${name} left their voice channel`)),
        });
    }
}
...
const UserContext: NavContextMenuPatchCallback = (children, { user }: UserContextProps) => {
    ...
    const label = isNotifyOn ? t("إيقاف الإشعار عند التغيّرات", "Don't notify on changes") : t("إشعار عند التغيّرات", "Notify on changes");
        ...
        PRESENCE_UPDATES({ updates }: { updates: PresenceUpdate[]; }) {
            ...
            for (const { user: { id: userId, username }, status, clientStatus } of updates) {
                ...
                if (lastStatuses.has(userId) && lastStatuses.get(userId) !== status) {
                    ...
                    showNotification({
                        title: shouldBeNative() ? t(`غيّر ${name} حالته`, `${name} changed status`) : t("تغيّر حالة مستخدم", "User status change"),
                        body: t(`أصبح الآن ${status}`, `They are now ${status}`),
                        noPersist: !settings.store.persistNotifications,
                        richBody: getRichBody(user, t(`حالة ${name} الآن ${status}`, `${name}'s status is now ${status}`)),
                    });
                }
