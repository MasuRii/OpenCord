...
export function TempMailModal({ modalProps }: { modalProps: RenderModalProps; }) {
    ...
    async function fetchInbox(acc: SavedAccount, silent = false) {
        ...
        try {
            ...
            if (!silent) setError(t("تعذّر تحميل الوارد: ", "Failed to load inbox: ") + e.message);
        } finally {
    ...
    async function openMessage(msg: TmMessage) {
        ...
        try {
            ...
            setError(t("تعذّر تحميل الرسالة: ", "Could not load message: ") + e.message);
        } finally {
            setMsgLoading(false);
        }
    }
    ...
    async function createAddress(random: boolean) {
        ...
        if (!domain) { setError(t("لا توجد نطاقات متاحة بعد.", "No domains available yet.")); return; }
        const user = random ? randomString(10) : customUser.trim();
        if (!user) { setError(t("أدخل اسم مستخدم أولاً.", "Enter a username first.")); return; }
        ...
        try {
            ...
            setView("inbox");
            setCustomUser("");
            showToast(t("تم الإنشاء: ", "Created: ") + address, Toasts.Type.SUCCESS);
        } catch (e: any) {
            setError(e.message);
        } finally {
            setLoading(false);
    ...
    return (
        <Modal {...modalProps} size="lg" title={t("البريد المؤقت", "Temp Mail")}>
            <div className="tm-shell">
                ...
                <div className="tm-sidebar">
                    <div className="tm-brand">
                        ...
                        <span>{t("البريد المؤقت", "Temp Mail")}</span>
                    </div>
                    ...
                    {active && (
                        <div className="tm-active-addr">
                            ...
                            <button className="tm-copy-btn" onClick={() => { copyToClipboard(active.address); showToast(t("تم النسخ!", "Copied!"), Toasts.Type.SUCCESS); }}>
                                Copy address
                            </button>
                        </div>
                    )}
                    ...
                    {view === "inbox" && (
                        <div className="tm-view">
                            <div className="tm-view-header">
                                ...
                                <button className="tm-icon-btn" title={t("تحديث", "Refresh")} onClick={() => active && fetchInbox(active)} disabled={loading}>
                                    ...
                                    {messages.map(m => (
                                        <div key={m.id} className={`tm-msg-row ${!m.seen ? "tm-msg-unread" : ""}`} onClick={() => openMessage(m)}>
                                            ...
                                            <div className="tm-msg-right">
                                                ...
                                                <button className="tm-del-btn" title={t("حذف", "Delete")} onClick={e => { e.stopPropagation(); handleDeleteMessage(m.id); }}>
                            ...
                            {openMsg && !msgLoading && (
                                <div className="tm-msg-view">
                                    ...
                                    <div className="tm-msg-view-meta">
                                        <span>{t("من ", "From ")}<strong>{openMsg.from.name || openMsg.from.address}</strong></span>
                                ...
                                {accounts.map(acc => (
                                    <div key={acc.id} className={`tm-account-card ${active?.id === acc.id ? "tm-account-active" : ""}`}>
                                        ...
                                        <div className="tm-account-btns">
                                            ...
                                            <button className="tm-icon-btn" title={t("نسخ", "Copy")} onClick={() => { copyToClipboard(acc.address); showToast(t("تم النسخ!", "Copied!"), Toasts.Type.SUCCESS); }}>
                                            ...
                                            <button className="tm-icon-btn tm-icon-btn-danger" title={t("حذف الحساب", "Delete account")} onClick={() => deleteAcc(acc)}>
                                                <svg viewBox="0 0 24 24" width={14} height={14} fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z" /></svg>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="tm-view">
                            ...
                            <div className="tm-new-section">
                                ...
                                <button className="tm-btn-primary" onClick={() => createAddress(true)} disabled={loading || !domains.length}>
                                    {loading ? t("جاري الإنشاء…", "Creating…") : t("⚡ توليد عنوان عشوائي", "⚡ Generate random address")}
                                </button>
                            </div>

                            <div className="tm-divider"><span>{t("أو اختر عنوانك", "or choose your own")}</span></div>
                            ...
                            <div className="tm-new-section">
                                ...
                                >
                                    {loading ? t("جاري الإنشاء…", "Creating…") : t("إنشاء عنوان", "Create address")}
                                </button>
                            </div>
                        </div>
                    )}
